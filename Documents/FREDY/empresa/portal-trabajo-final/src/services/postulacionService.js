 ...
 * Usado por VISTA_26_AplicacionesEmpresa (evita el "Cargando..." infinito)
 */
export const getAplicacionesConDetalle = async (idEmpresa, idEmpleo = null) => {
    let sql = `
        SELECT
            BIN_TO_UUID(p.id)          AS id,
            p.estado,
    ...
    const params = [idEmpresa];
    if (idEmpleo) {
        sql += ' AND e.id = UUID_TO_BIN(?)';
        params.push(idEmpleo);
    }
    sql += ' ORDER BY p.creadoEl DESC';
